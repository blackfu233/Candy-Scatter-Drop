...
class CandyOrdersScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image("sym-scatter", "assets/symbols/scatter.png?v=20260612scatterstar");
  ...
  grantScatterRewardFx(orderIndex = 0) {
    ...
    const icon = this.add.image(W - 62, startY, "sym-key").setDepth(34);
  ...
  createUi() {
    ...
    this.bonusBuyButton = this.add.rectangle(W / 2, 548, 236, 48, 0x19b7d4).setStrokeStyle(4, 0xffffff, 0.9);
    this.bonusBuyButton.setInteractive({ useHandCursor: true });
    this.bonusBuyLabel = this.add.text(W / 2, 548, "", {
      fontSize: 18,
      fontStyle: "900",
      color: "#ffffff",
      stroke: "#07354f",
  ...
  updateBetUi() {
    ...
    this.bonusBuyButton.setFillStyle(canBuyBonus ? 0x19b7d4 : 0x34606f);
    this.bonusBuyButton.setAlpha(canBuyBonus ? 1 : 0.68);
  ...
  updateOrders() {
    this.orders.forEach((order, i) => {
      ...
      if (order.scope === "free" && order.gold) {
        row.panel.setFillStyle(done ? 0xfff2a8 : near ? 0xffd94c : 0x9b6b16, done ? 0.36 : near ? 0.34 : 0.82);
      } else {
      ...
      if (order.scope !== "free" && order.rewardType === "scatter") {
        row.reward.setFontSize(10).setX(W - 60).setText("SCATTER");
      } else if (order.scope !== "free" && order.rewardType === "coinsScatter") {
        row.reward.setFontSize(10).setX(W - 62).setText(`${shownRange.min}-${shownRange.max}x+S`);
      } else {
